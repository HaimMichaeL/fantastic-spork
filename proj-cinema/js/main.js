...
function highlightAvailableSeats(){
    const elSeat = document.querySelector('.popup h2 span')
    const seat = elSeat.innerText.split('-')
   
    const rowIdx = +seat[0] - 1
    const colIdx = +seat[1] - 1
    const board = gCinema
   
    markSeats(rowIdx, colIdx, board)
   
    setTimeout(() => {
        unMarkSeats(rowIdx, colIdx, board)
    }, 2500)
   
}

function markSeats(rowIdx, colIdx, board){
    for (var i = rowIdx - 1; i <= rowIdx + 1; i++) {
        if (i < 0 || i >= board.length) continue
        for (var j = colIdx - 1; j <= colIdx + 1; j++) {
            if (i === rowIdx && j === colIdx)continue
            ...
            if (currCell.isSeat && !currCell.isBooked){
                currCell.isHighlight = true
                const elCurrSeat = document.querySelector(`[data-i="${i}"][data-j="${j}"]`)
                elCurrSeat.classList.add('highlight')
...
function unMarkSeats(rowIdx, colIdx, board){
    for (var i = rowIdx - 1; i <= rowIdx + 1; i++) {
        if (i < 0 || i >= board.length) continue
        for (var j = colIdx - 1; j <= colIdx + 1; j++) {
            if (i === rowIdx && j === colIdx) continue
            if (j < 0 || j >= board[0].length) continue
            var currCell = board[i][j]
            if (currCell.isSeat && !currCell.isBooked) currCell.isHighlight = false 
            const elCurrSeat = document.querySelector(`[data-i="${i}"][data-j="${j}"]`)
                elCurrSeat.classList.remove('highlight')
